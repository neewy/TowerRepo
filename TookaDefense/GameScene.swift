...
class GameScene: GameSceneInit {
    ...
    override func didFinishUpdate() {
        let enemies: [EnemyEntity] = entities.flatMap {
            entity in
        ...
        let towers: [TowerEntity] = entities.flatMap {
            entity in
        ...
        let slower: [ObstacleEntity] = entities.flatMap {
            entity in
...
//		enemy.spriteComponent.node.position = pointIsoTo2D(enemy.spriteComponent.node.position)
    ...
    func addObstacle(_ obstacleType: ObstacleType, gridPosition: int2) {
        if let node = graph.node(atGridPosition: gridPosition) {
            ...
            let obstacleEntity = ObstacleEntity(obstacleType: obstacleType, size: CGSize(width: GRID_SIZE * 1.7, height: GRID_SIZE * 1.7))
...
//			let location = point2DToIso(pointForGridPosition(node.gridPosition))
    ...
    func sortDepth() {
        ...
        //1
        let childrenSortedForDepth = isoView.children.sorted() {
